-- ============================================================
-- SCRIPT DE EXECUÇÃO E VALIDAÇÃO — Migration 0006 (Etapa 1B.5)
...
-- desfaz tudo, inclusive a criação/alteração da função e as duas
-- colunas novas, via ROLLBACK automático de uma exceção não capturada.
--
-- NÃO é a migration em si — a migration "de verdade" continua
-- sendo _etapa0/migrations/0006_v10_pendencia_financeira_reprocessamento.sql
-- (o corpo da função abaixo é uma cópia idêntica).
--
-- PASSO 0 (autenticação de teste), TESTE M (permissões por catálogo)
-- e TESTE L (não-admin rejeitado pela checagem interna) reaproveitam
-- exatamente a técnica já validada na migration 0005: set_config
-- local à transação com um admin real do próprio DEV. Ver o cabeçalho
-- do script 0005 para o histórico completo dessa correção.
--
-- NOVO nesta migration: TESTES N a T validam a pendência financeira
-- (Etapa 1B.5) — geração isolada por campo (status/valor/líquido,
-- N/O/P), não-duplicação de pendência já aberta (O), limpeza de
-- resolução anterior ao reabrir (P), venda não lançada nunca vira
-- pendência (Q), alteração inexistente não gera pendência (R),
-- rollback integral desfaz também a pendência (T), e o invariante
-- mais importante: public.rg_financeiro nunca é tocado por esta
-- função, nem quando pendências são geradas (TESTE S, por hash do
-- conteúdo da tabela antes/depois de todos os testes acima).
-- ============================================================
...
-- colunas novas de auditoria de resolução (aditivas, nullable)
...
DECLARE
  ...
  v_venda6 bigint; v_venda7 bigint; v_venda8 bigint; v_venda9 bigint; v_venda10 bigint; v_venda11 bigint;
  v_lote_novo bigint;
  v_resp jsonb;
...
BEGIN
  ...
  if v_admin_id is null then
    raise exception 'MIGRATION 0006: nenhum perfil admin aprovado encontrado em public.profiles neste ambiente — impossível testar rg_reprocessar_lote() com segurança. Nenhuma fixture foi criada.';
  end if;
  ...
  -- FIXTURES novas para os testes de pendência financeira (1B.5)
  insert into public.rg_vendas (transacao, produto, nome, status, valor, liquido, fin_lancado, lote_id, importacao_concluida, pendencia_financeira)
    values ('__TESTE1B5_TX6__','Produto Teste','Fulano','Completo',300.00,270.00,true,v_lote_original,true,false) returning id into v_venda6;
  insert into public.rg_vendas (transacao, produto, nome, status, valor, liquido, fin_lancado, lote_id, importacao_concluida, pendencia_financeira)
    values ('__TESTE1B5_TX7__','Produto Teste','Fulano','Completo',400.00,360.00,true,v_lote_original,true,true) returning id into v_venda7;
  insert into public.rg_vendas (transacao, produto, nome, status, valor, liquido, fin_lancado, lote_id, importacao_concluida, pendencia_financeira, pendencia_financeira_resolvida_por, pendencia_financeira_resolvida_em)
    values ('__TESTE1B5_TX8__','Produto Teste','Fulano','Completo',500.00,450.00,true,v_lote_original,true,false, v_admin_id, now() - interval '1 day') returning id into v_venda8;
  insert into public.rg_vendas (transacao, produto, nome, status, valor, liquido, fin_lancado, lote_id, importacao_concluida, pendencia_financeira)
    values ('__TESTE1B5_TX9__','Produto Teste','Fulano','Completo',600.00,540.00,false,v_lote_original,true,false) returning id into v_venda9;
  insert into public.rg_vendas (transacao, produto, nome, status, valor, liquido, fin_lancado, lote_id, importacao_concluida, pendencia_financeira)
    values ('__TESTE1B5_TX10__','Produto Teste','Fulano','Completo',700.00,630.00,true,v_lote_original,true,false) returning id into v_venda10;
  insert into public.rg_vendas (transacao, produto, nome, status, valor, liquido, fin_lancado, lote_id, importacao_concluida, pendencia_financeira)
    values ('__TESTE1B5_TX11__','Produto Teste','Fulano','Completo',850.00,765.00,true,v_lote_original,true,false) returning id into v_venda11;
  v_vendas_teste := v_vendas_teste || array[v_venda6, v_venda7, v_venda8, v_venda9, v_venda10, v_venda11];

  raise notice '=== fixtures: lote_original=%, lote_outro=%, lote_vazio=%, vendas=%,%,%,%(excluída),%(outro lote),%(lançada),%(já pendente),%(resolvida antes),%(não lançada),%(lançada),%(lançada p/ rollback) ===',
    v_lote_original, v_lote_outro, v_lote_vazio, v_venda1, v_venda2, v_venda3, v_venda4_excluida, v_venda5_outro_lote, v_venda6, v_venda7, v_venda8, v_venda9, v_venda10, v_venda11;
  ...
  if (v_resp->>'ok')::boolean and (v_resp->>'atualizadas')::int = 1 and (v_resp->>'alteracoes_status')::int = 1
     ...
     and (v_resp->>'pendencias_financeiras')::int = 0 and v_venda_check.pendencia_financeira = false
  ...
  -- restaura o contexto admin para o restante do script
  perform set_config('request.jwt.claim.sub', v_admin_id::text, true);
  perform set_config('request.jwt.claims', jsonb_build_object('sub', v_admin_id::text, 'role', 'authenticated')::text, true);

  -- captura o estado de rg_financeiro ANTES dos testes de pendência,
  -- para o TESTE S (invariante: função nunca toca em rg_financeiro)
  select md5(coalesce(string_agg(t::text, '|' order by id), '')) into v_financeiro_antes from public.rg_financeiro t;

  -- TESTE N: venda já lançada sofre alteração real → pendência nova
  insert into public.rg_vendas_lotes (arquivo_nome, status, reprocessa_lote_id) values ('__teste_N__.csv','processando', v_lote_original) returning id into v_lote_novo;
  v_lotes_teste := v_lotes_teste || v_lote_novo;
  v_resp := public.rg_reprocessar_lote(v_lote_original, v_lote_novo, 1,
    jsonb_build_array(jsonb_build_object('venda_id', v_venda6, 'status', 'Aprovado', 'valor', '300.00', 'liquido', '270.00')), '[]'::jsonb);
  select * into v_venda_check from public.rg_vendas where id = v_venda6;
  select * into v_lote_check from public.rg_vendas_lotes where id = v_lote_novo;
  if (v_resp->>'ok')::boolean and (v_resp->>'pendencias_financeiras')::int = 1
     and v_venda_check.pendencia_financeira = true
     and v_venda_check.pendencia_financeira_resolvida_por is null and v_venda_check.pendencia_financeira_resolvida_em is null
     and v_lote_check.pendencias_financeiras = 1
  then raise notice 'TESTE N (status — pendência nova gerada) aprovado';
  else v_falhas := v_falhas + 1; v_motivos := v_motivos || E'\n- TESTE N reprovado: ' || v_resp::text; end if;

  -- TESTE O: venda já pendente sofre nova alteração de VALOR →
  -- continua pendente, mas não conta de novo nesta chamada
  insert into public.rg_vendas_lotes (arquivo_nome, status, reprocessa_lote_id) values ('__teste_O__.csv','processando', v_lote_original) returning id into v_lote_novo;
  v_lotes_teste := v_lotes_teste || v_lote_novo;
  v_resp := public.rg_reprocessar_lote(v_lote_original, v_lote_novo, 1,
    jsonb_build_array(jsonb_build_object('venda_id', v_venda7, 'status', 'Completo', 'valor', '410.00', 'liquido', '360.00')), '[]'::jsonb);
  select * into v_venda_check from public.rg_vendas where id = v_venda7;
  if (v_resp->>'ok')::boolean and (v_resp->>'alteracoes_valor')::int = 1 and (v_resp->>'pendencias_financeiras')::int = 0
     and v_venda_check.pendencia_financeira = true and v_venda_check.valor = 410.00
  then raise notice 'TESTE O (valor — pendência já aberta, não recontada) aprovado';
  else v_falhas := v_falhas + 1; v_motivos := v_motivos || E'\n- TESTE O reprovado: ' || v_resp::text; end if;

  -- TESTE P: venda com resolução anterior sofre nova alteração de
  -- LÍQUIDO → reabre a pendência e limpa resolvida_por/resolvida_em antigos
  insert into public.rg_vendas_lotes (arquivo_nome, status, reprocessa_lote_id) values ('__teste_P__.csv','processando', v_lote_original) returning id into v_lote_novo;
  v_lotes_teste := v_lotes_teste || v_lote_novo;
  v_resp := public.rg_reprocessar_lote(v_lote_original, v_lote_novo, 1,
    jsonb_build_array(jsonb_build_object('venda_id', v_venda8, 'status', 'Completo', 'valor', '500.00', 'liquido', '460.00')), '[]'::jsonb);
  select * into v_venda_check from public.rg_vendas where id = v_venda8;
  if (v_resp->>'ok')::boolean and (v_resp->>'pendencias_financeiras')::int = 1
     and v_venda_check.pendencia_financeira = true
     and v_venda_check.pendencia_financeira_resolvida_por is null and v_venda_check.pendencia_financeira_resolvida_em is null
  then raise notice 'TESTE P (líquido — limpa resolução anterior ao reabrir pendência) aprovado';
  else v_falhas := v_falhas + 1; v_motivos := v_motivos || E'\n- TESTE P reprovado: ' || v_resp::text; end if;

  -- TESTE Q: venda não lançada sofre alteração → nunca vira pendência
  insert into public.rg_vendas_lotes (arquivo_nome, status, reprocessa_lote_id) values ('__teste_Q__.csv','processando', v_lote_original) returning id into v_lote_novo;
  v_lotes_teste := v_lotes_teste || v_lote_novo;
  v_resp := public.rg_reprocessar_lote(v_lote_original, v_lote_novo, 1,
    jsonb_build_array(jsonb_build_object('venda_id', v_venda9, 'status', 'Aprovado', 'valor', '600.00', 'liquido', '540.00')), '[]'::jsonb);
  select * into v_venda_check from public.rg_vendas where id = v_venda9;
  if (v_resp->>'ok')::boolean and (v_resp->>'pendencias_financeiras')::int = 0
     and v_venda_check.pendencia_financeira = false
  then raise notice 'TESTE Q (venda não lançada nunca vira pendência) aprovado';
  else v_falhas := v_falhas + 1; v_motivos := v_motivos || E'\n- TESTE Q reprovado: ' || v_resp::text; end if;

  -- TESTE R: venda lançada, mas SEM alteração real → não vira pendência
  insert into public.rg_vendas_lotes (arquivo_nome, status, reprocessa_lote_id) values ('__teste_R__.csv','processando', v_lote_original) returning id into v_lote_novo;
  v_lotes_teste := v_lotes_teste || v_lote_novo;
  v_resp := public.rg_reprocessar_lote(v_lote_original, v_lote_novo, 1,
    jsonb_build_array(jsonb_build_object('venda_id', v_venda10, 'status', 'Completo', 'valor', '700.00', 'liquido', '630.00')), '[]'::jsonb);
  select * into v_venda_check from public.rg_vendas where id = v_venda10;
  if (v_resp->>'ok')::boolean and (v_resp->>'inalteradas')::int = 1 and (v_resp->>'pendencias_financeiras')::int = 0
     and v_venda_check.pendencia_financeira = false
  then raise notice 'TESTE R (inalterada não gera pendência mesmo lançada) aprovado';
  else v_falhas := v_falhas + 1; v_motivos := v_motivos || E'\n- TESTE R reprovado: ' || v_resp::text; end if;

  -- TESTE T: rollback integral também desfaz a pendência financeira —
  -- um item que geraria pendência + um item inválido no mesmo payload
  insert into public.rg_vendas_lotes (arquivo_nome, status, reprocessa_lote_id) values ('__teste_T__.csv','processando', v_lote_original) returning id into v_lote_novo;
  v_lotes_teste := v_lotes_teste || v_lote_novo;
  v_resp := public.rg_reprocessar_lote(v_lote_original, v_lote_novo, 2,
    jsonb_build_array(
      jsonb_build_object('venda_id', v_venda11, 'status', 'Aprovado', 'valor', '800.00', 'liquido', '720.00'),
      jsonb_build_object('venda_id', v_venda5_outro_lote, 'status', 'Completo', 'valor', '20.00', 'liquido', '18.00')
    ), '[]'::jsonb);
  select * into v_venda_check from public.rg_vendas where id = v_venda11;
  select count(*) into v_residual from public.rg_vendas_alteracoes_historico where venda_id = v_venda11 and lote_id = v_lote_novo;
  if not (v_resp->>'ok')::boolean
     and v_venda_check.status = 'Completo' and v_venda_check.valor = 850.00 and v_venda_check.liquido = 765.00
     and v_venda_check.pendencia_financeira = false
     and v_venda_check.ultima_atualizacao_lote_id is null and v_residual = 0
  then raise notice 'TESTE T (rollback integral também desfaz pendência financeira) aprovado — venda11 intacta, sem pendência residual';
  else v_falhas := v_falhas + 1; v_motivos := v_motivos || E'\n- TESTE T reprovado: resp=' || v_resp::text || ', residual=' || v_residual; end if;

  -- TESTE S: invariante mais importante — rg_financeiro nunca é
  -- tocado por esta função, nem quando pendências são geradas
  ...
  then raise notice 'TESTE S (rg_financeiro intocado durante todos os testes de pendência) aprovado';
  else v_falhas := v_falhas + 1; v_motivos := v_motivos || E'\n- TESTE S reprovado: rg_financeiro mudou durante os testes de pendência'; end if;

  raise notice '=== TESTE J (RLS real, role authenticated não-admin): não testável com segurança pelo SQL Editor, que ignora RLS por rodar com role privilegiada — TESTE L acima cobre só a checagem interna da função. Teste via app continua obrigatório antes de considerar a Etapa 1B.5 encerrada. ===';
  ...
  -- ===== VEREDITO FINAL =====
  if v_falhas > 0 then
    raise exception 'MIGRATION 0006: % teste(s) reprovado(s):%', v_falhas, v_motivos;
  end if;

  raise notice 'TODOS OS TESTES DA MIGRATION 0006 FORAM APROVADOS';
END $$;
...
-- inclusive desfazendo a criação/alteração da função e as colunas novas.
