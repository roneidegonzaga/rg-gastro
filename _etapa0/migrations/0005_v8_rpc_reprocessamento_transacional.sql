-- ============================================================
-- MIGRATION 0005 — v8: RPC de reprocessamento transacional (Etapa 1B.3)
-- Primeira funcao RPC de negocio do projeto. Nenhuma tabela/coluna
-- nova — só a função e o ajuste de permissão de execução.
--
-- IMPORTANTE sobre permissões: o Supabase concede EXECUTE em toda
-- função nova no schema public a anon/authenticated/service_role
-- via ALTER DEFAULT PRIVILEGES do próprio projeto — um simples
-- "REVOKE ... FROM PUBLIC" não remove esses grants explícitos e
-- individuais (confirmado empiricamente no DEV: anon e service_role
-- continuaram com EXECUTE mesmo após o REVOKE FROM PUBLIC). Por
-- isso o REVOKE abaixo nomeia public, anon e service_role
-- explicitamente.
--
...
-- 0004_v7_trava_importacao_parcial.sql (public.rg_vendas_lotes,
-- public.rg_vendas.lote_id/excluido/importacao_concluida,
-- public.rg_vendas_alteracoes_historico, public.rg_is_admin())
-- ============================================================
